...
#normal and bidirectional model paths
...
#mean and standard deviation for de-standardization later
...
def evaluate(rank, world_size, bidirectional):
    ...
    if rank == 0:
        if bidirectional == "true":
            os.makedirs("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/1Day/", exist_ok=True)
        if bidirectional == "false":
            os.makedirs("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/1Day/", exist_ok=True)

    if bidirectional == "true":
        csv_path = "/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/1Day/LSTM_bidirectional_eval_per_sample_1day_server_" + str(rank) + ".csv"
        #setting up csv to write RMSEs to
        with open(csv_path, "w", newline="") as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow(["GlobalSampleID", "Step", "Real_NDVI_RMSE"])
    if bidirectional == "false":
        csv_path = "/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/1Day/LSTM_eval_per_sample_1day_server_" + str(rank) + ".csv"
    ...
    with torch.no_grad():
        for batch_idx, (x, y) in enumerate(test_loader):
            ...
            #autoregressive eval loop, feeds prediction back as input and stores in stack
            ...
            for i in range(B):
                for t in range(OFFSET):
                    ...
                    if bidirectional == "true":
                        ...
                        if rank == 0:
                            mpimg.imsave(f"/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/1Day/LSTM_bidirectional_sample{batch_idx * B + i:05d}_day{t+1}_pred_1day.png", pred_ndvi, cmap='viridis', vmin=-1, vmax=1)
                            mpimg.imsave(f"/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/1Day/LSTM_bidirectional_sample{batch_idx * B + i:05d}_day{t+1}_true_1day.png", target_ndvi, cmap='viridis', vmin=-1, vmax=1)

                        #Log RMSE for this sample
                        with open("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/1Day/LSTM_bidirectional_per_sample_log_1day.txt", "a") as logf:
                    ...
                    if bidirectional == "false":
                        ...
                        with open (csv_path, "a", newline="") as csv_file:
                            ...
                            writer.writerow([global_sample_idx, t+1, pixel_rmse])
                        if rank == 0:
                            mpimg.imsave(f"/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/1Day/LSTM_sample{batch_idx * B + i:05d}_day{t+1}_pred_1day.png", pred_ndvi, cmap='viridis', vmin=-1, vmax=1)
                            mpimg.imsave(f"/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/1Day/LSTM_sample{batch_idx * B + i:05d}_day{t+1}_true_1day.png", target_ndvi, cmap='viridis', vmin=-1, vmax=1)

                        #Log RMSE for this sample
                        with open("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/1Day/LSTM_per_sample_log_1day.txt", "a") as logf:
                            logf.write(f"Sample {i} - Day {t+1}: Real NDVI RMSE = {pixel_rmse:.4f}\n")

            #per step RMSE averaging and de-standardizing
            for step in range(OFFSET):
                y_step_std = y[:, step, :, :]
...
#logging stuff
    if rank == 0:
        ...
        if bidirectional == "true":
            with open("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/1Day/LSTM_bidirectional_evaluation_log_1Day.txt", "a") as f:
        ...
        if bidirectional == "false":
            with open("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/1Day/LSTM_evaluation_log_1Day.txt", "a") as f:
                f.write(f"[{datetime.datetime.now()}] Final average Real NDVI RMSE: {final_avg_rmse:.4f}\n")
                for step, rmse_val in enumerate(avg_rmse_per_step.tolist(), 1):
                    f.write(f"Step {step} Real NDVI RMSE: {rmse_val:.4f}\n")
