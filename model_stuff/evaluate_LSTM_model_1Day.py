...
TIME_STEPS = 1  # Match training window
OFFSET = 8
...
def evaluate(rank, world_size, bidirectional):
            ...
            if rank == 0 and total_batches == 0:
                ...
                for i in range(min(3, B)):  #Save first 3 samples
                    for t in range(OFFSET):
                        ...
                        #Save images
                        if bidirectional:
                            mpimg.imsave(f"/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/1Day/sample{i}_day{t+1}_pred.png", pred_ndvi, cmap='viridis', vmin=-1, vmax=1)
                            mpimg.imsave(f"/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/1Day/sample{i}_day{t+1}_true.png", target_ndvi, cmap='viridis', vmin=-1, vmax=1)

                            #Log RMSE for this sample
                            with open("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/1Day/sample_log.txt", "a") as logf:
                                logf.write(f"Sample {i} - Day {t+1}: RMSE = {pixel_rmse:.4f}\n")
                        else:
                            mpimg.imsave(f"/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/1Day/sample{i}_day{t+1}_pred.png", pred_ndvi, cmap='viridis', vmin=-1, vmax=1)
                            mpimg.imsave(f"/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/1Day/sample{i}_day{t+1}_true.png", target_ndvi, cmap='viridis', vmin=-1, vmax=1)

                            #Log RMSE for this sample
                            with open("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/1Day/sample_log.txt", "a") as logf:
                                logf.write(f"Sample {i} - Day {t+1}: RMSE = {pixel_rmse:.4f}\n")
    ...
    if rank == 0:
        ...
        if bidirectional:
            with open("/s/bach/b/class/cs535/cs535a/data/new_models/LSTM_bidirectional_evaluation_log_1Day.txt", "a") as f:
                f.write(f"[{datetime.datetime.now()}] Final average RMSE: {final_avg_rmse:.4f}\n")
                for step, rmse_val in enumerate(avg_rmse_per_step.tolist(), 1):
                    f.write(f"Step {step} RMSE: {rmse_val:.4f}\n")
        else:
            with open("/s/bach/b/class/cs535/cs535a/data/new_models/LSTM_evaluation_log_1Day.txt", "a") as f:
                f.write(f"[{datetime.datetime.now()}] Final average RMSE: {final_avg_rmse:.4f}\n")
            for step, rmse_val in enumerate(avg_rmse_per_step.tolist(), 1):
                f.write(f"Step {step} RMSE: {rmse_val:.4f}\n")
