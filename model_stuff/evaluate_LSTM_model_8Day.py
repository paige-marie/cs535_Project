...
#normal and bidirectional lstm model paths
...
#grabbing mean and standard deviation for de-standardization
...
#partitioning batch size for nodes
...
def evaluate(rank, world_size, bidirectional):
    #making sure the right parameters are grabbed
    if bidirectional == "true":
        with open("/s/bach/b/class/cs535/cs535a/data/shuffled_data_by_window/8day/hyperparamters/8day_lstm_bidirectional_config.json") as f:
            config = json.load(f)
    ...
    #make sure the right model is used
    if bidirectional == "true":
    ...
    if rank == 0:
        if bidirectional == "true":
            os.makedirs("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/8Day/", exist_ok=True)
        if bidirectional == "false":
            os.makedirs("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/8Day/", exist_ok=True)

    if bidirectional == "true":
        csv_path = "/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/8Day/LSTM_bidirectional_eval_per_sample_8day_server_" + str(rank) + ".csv"
        #setting up csv writer for the per sample results
        with open(csv_path, "w", newline="") as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow(["GlobalSampleID", "Step", "Real_NDVI_RMSE"])
    if bidirectional == "false":
        csv_path = "/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/8Day/LSTM_eval_per_sample_8day_server_" + str(rank) + ".csv"
    ...
    with torch.no_grad():
        for batch_idx, (x, y) in enumerate(test_loader):
            ...
            #autoregressive eval loop, feeding predicted NDVI image as input and saving as a stack
            ...
            #assessment of how autoregressive prediction did, and image generation
            for i in range(B):
                for t in range(OFFSET):
                    ...
                    #Save images, only for master node
                    if bidirectional == "true":
                        ...
                        if rank == 0:
                            mpimg.imsave(f"/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/8Day/LSTM_bidirectional_sample{batch_idx * B + i:05d}_day{t+1}_pred_8day.png", pred_ndvi, cmap='viridis', vmin=-1, vmax=1)
                            mpimg.imsave(f"/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/8Day/LSTM_bidirectional_sample{batch_idx * B + i:05d}_day{t+1}_true_8day.png", target_ndvi, cmap='viridis', vmin=-1, vmax=1)

                        #Log RMSE for this sample
                        with open("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM_bidirectional/8Day/LSTM_bidirectional_per_sample_log.txt", "a") as logf:
                    ...
                    if bidirectional == "false":
                        ...
                        with open (csv_path, "a", newline="") as csv_file:
                            ...
                            writer.writerow([global_sample_idx, t+1, pixel_rmse])
                        if rank == 0:
                            mpimg.imsave(f"/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/8Day/LSTM_sample{batch_idx * B + i:05d}_day{t+1}_pred_8day.png", pred_ndvi, cmap='viridis', vmin=-1, vmax=1)
                            mpimg.imsave(f"/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/8Day/LSTM_sample{batch_idx * B + i:05d}_day{t+1}_true_8day.png", target_ndvi, cmap='viridis', vmin=-1, vmax=1)

                        #Log RMSE for this sample
                        with open("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/8Day/LSTM_per_sample_log_8day.txt", "a") as logf:
                            logf.write(f"Sample {i} - Day {t+1}: RMSE = {pixel_rmse:.4f}\n")
...
#logging and printing stuff
    if rank == 0:
        ...
        if bidirectional == "true":
            with open("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/8Day/LSTM_bidirectional_evaluation_log_8Day.txt", "a") as f:
        ...
        if bidirectional == "false":
            with open("/s/bach/b/class/cs535/cs535a/data/eval_results/LSTM/8Day/LSTM_evaluation_log_8Day.txt", "a") as f:
                f.write(f"[{datetime.datetime.now()}] Final average Real NDVI RMSE: {final_avg_rmse:.4f}\n")
                for step, rmse_val in enumerate(avg_rmse_per_step.tolist(), 1):
                    f.write(f"Step {step} Real NDVI RMSE: {rmse_val:.4f}\n")
