...
OUT_DIR = "/s/bach/b/class/cs535/cs535a/data/preprocessed_1day/"

## must be changed along with nam for the different time windows we want
TIME_STEPS = 1
OFFSET = 1
...
for i in range(len(all_files) - TIME_STEPS - OFFSET + 1):
    ...
    for tif in sequence_paths:
        with rasterio.open(os.path.join(DATA_DIR, tif)) as src:
            ...
            bands = np.stack([img[2], img[1], img[0]])  # [NDVI, P, T]
    ...
    with rasterio.open(os.path.join(DATA_DIR, target_path)) as src:
        ndvi_target = src.read(3)  # Band 3 = NDVI
